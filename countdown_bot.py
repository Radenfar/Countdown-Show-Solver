...
def countdown_solver(target, unsorted_numbers):
    try:
...
def word_round(english_words, letters):
    new_letters = [x.lower() for x in letters]
    potentials = []
    for word in english_words:
        word_split = list(word)
        if all(item in new_letters for item in word_split) == True:
...
#main routine
while True:
    print("Enter 1 for word round, enter 2 for numbers round:")
    ui = input('-: ')
    if ui == '1':
        ...
        for i in range(9):
            ui_letters = input('-: ')
            letters.append(ui_letters)
        print(word_round(english_words, letters))
    elif ui == '2':
        print("Enter your numbers:")
        numbers = []
        for i in range(6):
            ui_numbers = int(input('-: '))
            numbers.append(ui_numbers)
